...
S_oahValuedAtom xml2brlOah::handleAtom (
  ostream&  os,
  S_oahAtom atom)
{
  ...
  if (
    // version atom?
    S_xml2brlOahVersionAtom
      versionAtom =
        dynamic_cast<xml2brlOahVersionAtom*>(&(*atom))
    ) {
#ifdef TRACE_OAH
    if (gExecutableOah->fTraceOah && ! gGeneralOah->fQuiet) {
      os <<
    ...
    // about atom?
    S_xml2brlOahAboutAtom
      aboutAtom =
        dynamic_cast<xml2brlOahAboutAtom*>(&(*atom))
    ) {
#ifdef TRACE_OAH
    if (gExecutableOah->fTraceOah && ! gGeneralOah->fQuiet) {
      os <<
    ...
    // contact atom?
    S_xml2brlOahContactAtom
      contactAtom =
        dynamic_cast<xml2brlOahContactAtom*>(&(*atom))
    ) {
#ifdef TRACE_OAH
    if (gExecutableOah->fTraceOah && ! gGeneralOah->fQuiet) {
      os <<
