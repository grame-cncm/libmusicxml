...
//#define USE_DUAL_HANDLE
  ...
  // ------------------------------------------------------
  S_xml2bmmlOahDualHandler dualHandler;

  try {
    dualHandler =
      xml2bmmlOahDualHandler::create (
        executableName,
        gOutputOstream);
  }
  ...
  S_xml2bmmlInsiderOahHandler handler;
  ...
  if (gTraceOah->getTracePasses ()) {
    ...
    gLogOstream <<
      "This is xml2bmml " << currentVersionNumber () <<
  ...
  if (gIndenter != 0) {
    gLogOstream <<
      "### xml2bmml gIndenter final value: "<< gIndenter.getIndent () << " ###" <<
      endl <<
      endl;
  }
  ...
  if (err != kNoErr) {
    gLogOstream <<
      "### Conversion from MusicXML to BMML code failed ###" <<
      endl <<
      endl;
