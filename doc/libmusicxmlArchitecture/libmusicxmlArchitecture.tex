...
The picture at figure \ref {Architecture}, page \pageref {Architecture}, shows how \lib\ is structured. The light gray, dashed arrows indicate items not yet available. 
...
\texttt{16} & converts an meielement tree into MSR data -- in design phase;
\tabularnewline

\texttt{17} & converts MSR data into an meielement tree -- in design phase.
\tabularnewline
...
The converters provided by \lib\ are in the form of functions. Executable command-line applications are also supplied:
%\begin{adjustwidth}{-0.5cm}{-0.5cm}
\begin{center}
\footnotesize
\def \contentsWidth{0.7\textwidth}
...
New converters involving \mei\ will be available as as soon as passes 15 and 16, converting MEI element trees into MSR and back, are added to \lib. This will add the following converters to \lib:
%\begin{adjustwidth}{-0.5cm}{-0.5cm}
\begin{center}
\footnotesize
\def \contentsWidth{0.7\textwidth}
...
\hline\\[-3.0ex]
%
\meiToGuido & converts \mei\ data to Guido code, using passes:

\tab\ 13 $\Rightarrow$ 16 $\Rightarrow$ 15 $\Rightarrow$ 3
\tabularnewline

\meiToXml & converts \mei\ data to Guido code, using passes:

\tab\ 13 $\Rightarrow$ 15 $\Rightarrow$ 12 $\Rightarrow$ 2
\tabularnewline

\meiToMei & converts \mei\ data to MSR and back:

\tab\ 13 $\Rightarrow$ 16 $\Rightarrow$ 17 $\Rightarrow$ 14
\tabularnewline

\meiToLy & performs the 4 hops from \mei\ to \lily\ to translate the former into the latter, using these passes:
...
\meiToBrl & performs the 5 hops from \mei\ to \braille\ to translate the former into the latter (draft):
