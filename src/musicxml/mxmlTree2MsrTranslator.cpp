...
*/
  ...
  if (gGlobalMxmlTree2MsrOah->fPartsOmitIDSet.size ()) {
    set<string>::iterator
      it =
        gGlobalMxmlTree2MsrOah->fPartsOmitIDSet.find (
          fCurrentPart->
            getPartID ());

    if (it != gGlobalMxmlTree2MsrOah->fPartsOmitIDSet.end ()) {
  ...
  if (gGlobalMxmlTree2MsrOah->fPartsKeepIDSet.size ()) {
    set<string>::iterator
      it =
        gGlobalMxmlTree2MsrOah->fPartsKeepIDSet.find (
          fCurrentPart->
            getPartID ());

    if (it == gGlobalMxmlTree2MsrOah->fPartsKeepIDSet.end ()) {
  ...
  if (gGlobalMxmlTree2MsrOah->fPartsOmitNameSet.size ()) {
    set<string>::iterator
      it =
        gGlobalMxmlTree2MsrOah->fPartsOmitNameSet.find (
          fCurrentPart->
            getPartName ());

    if (it != gGlobalMxmlTree2MsrOah->fPartsOmitNameSet.end ()) {
      // the simplest way to omit this part
      // is to remove it from its part-group
      // now that is has been completely built and populated
      fCurrentPart->
  ...
  if (gGlobalMxmlTree2MsrOah->fPartsKeepNameSet.size ()) {
    set<string>::iterator
      it =
        gGlobalMxmlTree2MsrOah->fPartsKeepNameSet.find (
          fCurrentPart->
            getPartName ());

    if (it == gGlobalMxmlTree2MsrOah->fPartsKeepNameSet.end ()) {
      // the simplest way not to keep this part
      // is to remove it from its part-group
      // now that is has been completely built and populated
      fCurrentPart->
...
void mxmlTree2MsrTranslator::visitEnd ( S_clef& elt )
{
  ...
  map<msrClefKind, msrClefKind>::const_iterator
    it =
      gGlobalMxmlTree2MsrOah->fReplaceClefMapVariable.find (clefKind);

  if (it != gGlobalMxmlTree2MsrOah->fReplaceClefMapVariable.end ()) {
...
/* JMI
  ...
  */
  ...
  if (! gGlobalMxmlTree2MsrOah->fOmitWords) {
    bool wordsHasBeenHandled = false;

    if (gGlobalMxmlTree2MsrOah->fConvertWordsToDalSegno.size ()) {
      // is wordsValue in the string to dal segno kind map?
      map<string, msrDalSegno::msrDalSegnoKind>::iterator
        it =
          gGlobalMxmlTree2MsrOah->fConvertWordsToDalSegno.find (wordsValue);

      if (it != gGlobalMxmlTree2MsrOah->fConvertWordsToDalSegno.end ()) {
...
#ifdef TRACE_OAH
        if (gGlobalTraceOahGroup->fTraceWords || gGlobalTraceOahGroup->fTraceDalSegnos) {
          fLogOutputStream <<
            "Converting words '" <<
            wordsValue <<
...
#ifdef TRACE_OAH
    if (gGlobalTraceOahGroup->fTraceTempos || gGlobalTraceOahGroup->fTraceBeams) {
      fLogOutputStream <<
...
// <tied orientation="over" type="start"/>
  ...
  if (fCurrentTieKind != msrTie::kTieNone) {
    if (! gGlobalMxmlTree2MsrOah->fOmitTies) {
      fCurrentTie =
        msrTie::create (
          inputLineNumber,
          fCurrentTieKind);
...
#ifdef TRACE_OAH
  if (gGlobalTraceOahGroup->fTraceSlursDetails) {
...
/* JMI BUGGED?
  ...
  */
  ...
  // color JMI
    ...
    if (! gGlobalMxmlTree2MsrOah->fOmitSlurs) {
      S_msrSlur
        slur =
          msrSlur::create (
            inputLineNumber,
...
void mxmlTree2MsrTranslator::visitStart ( S_wedge& elt )
{
  ...
  if (gGlobalMxmlTree2MsrOah->fAllWedgesBelow) {
    fCurrentDirectionPlacementKind = kPlacementBelow;
  }

  if (! gGlobalMxmlTree2MsrOah->fOmitWedges) {
    S_msrWedge
      wedge =
        msrWedge::create (
          inputLineNumber,
...
#ifdef TRACE_OAH
  if (gGlobalTraceOahGroup->fTraceLyricsDetails) {
    fLogOutputStream <<
...
#ifdef TRACE_OAH
// JMI  if (gGlobalTraceOahGroup->fTraceMeasures) {
  if (gGlobalTraceOahGroup->getTracePasses ()) {
    fLogOutputStream <<
      endl <<
...
#ifdef TRACE_OAH
  if (gGlobalTraceOahGroup->fTraceDetailed) {
    set<int>::const_iterator
      it =
        gGlobalGeneralOahGroup->
          fTraceDetailedMeasureNumbersSet.find (fMeasuresCounter);

    if (it != gGlobalTraceOahGroup->fTraceDetailedMeasureNumbersSet.end ()) {
      // yes, activate detailed trace for it
      gGlobalMxmlTree2MsrOah = gGlobalMxmlTree2MsrOahWithDetailedTrace;
      gGlobalGeneralOahGroup  = gGlobalGeneralOahGroupWithDetailedTrace;
      gGlobalMsrOah      = gGlobalMsrOahWithDetailedTrace;
      gGlobalLpsrOah     = gGlobalLpsrOahWithDetailedTrace;
      gGlobalLpsr2LilypondOah = gGlobalLilypondOahWithDetailedTrace;
    }
  }
#endif
  */
...
//      fVoicesLastMetNoteMap [currentVoice];
  ...
  if (gGlobalMxmlTree2MsrOah->fAddEmptyMeasuresStringToIntMap.size ()) {
  //  if (! fOnGoingRestMeasures) { JMI
      // should we add empty measures after current measures?
      map<string,int>::const_iterator
        it =
          gGlobalMxmlTree2MsrOah->fAddEmptyMeasuresStringToIntMap.find (
            fCurrentMeasureNumber);

      if (it != gGlobalMxmlTree2MsrOah->fAddEmptyMeasuresStringToIntMap.end ()) {
...
/* JMI
  ...
  gGlobalMxmlTree2MsrOah = gGlobalMxmlTree2MsrOahUserChoices;
  gGlobalGeneralOahGroup  = gGlobalGeneralOahGroupUserChoices;
  gGlobalMsrOah      = gGlobalMsrOahUserChoices;
  gGlobalLpsrOah     = gGlobalLpsrOahUserChoices;
  gGlobalLpsr2LilypondOah = gGlobalLilypondOahUserChoices;
  */
}
...
void mxmlTree2MsrTranslator::visitStart ( S_accent& elt )
{
  ...
  if (! gGlobalMxmlTree2MsrOah->fOmitArticulations) {
...
void mxmlTree2MsrTranslator::visitStart ( S_trill_mark& elt )
{
 ...
 // type : upright inverted  (Binchois20.xml) JMI
  ...
  if (! gGlobalMxmlTree2MsrOah->fOmitOrnaments) {
...
void mxmlTree2MsrTranslator::visitStart( S_f& elt)
{
  ...
  // should the placement be forced to 'below'?
  if (gGlobalMxmlTree2MsrOah->fAllDynamicsBelow) {
    placementKind = kPlacementBelow;
  }

  // create the dynamics
  if (! gGlobalMxmlTree2MsrOah->fOmitDynamics) {
    S_msrDynamics
      dynamics =
        msrDynamics::create (
          inputLineNumber,
...
void mxmlTree2MsrTranslator::visitStart ( S_grace& elt )
{
  ...
  if (gGlobalMxmlTree2MsrOah->fSlashAllGraceNotes) {
    fCurrentGraceIsSlashed = true;
  }

  // should all grace notes be beamed?
  if (gGlobalMxmlTree2MsrOah->fBeamAllGraceNotes) {
...
#ifdef TRACE_OAH
  if (gGlobalTraceOahGroup->fTraceChordsDetails) {
  ...
  S_msrNote note, S_msrChord chord)
{
  ...
  if (noteStem) {
#ifdef TRACE_OAH
    if (gGlobalTraceOahGroup->fTraceStems) {
      fLogOutputStream <<
        "Copying stem '" <<
        noteStem->asString () <<
        "' from note " << note->asString () <<
  ...
  S_msrNote note, S_msrChord chord)
{
  ...
  if (noteTie) {
#ifdef TRACE_OAH
    if (gGlobalTraceOahGroup->fTraceTies) {
      fLogOutputStream <<
        "Appending tie '" <<
        noteTie->asString () <<
        "' from note " << note->asString () <<
...
#ifdef TRACE_OAH
  if (gGlobalTraceOahGroup->fTraceTupletsDetails) {
    displayTupletsStack (
  ...
  S_msrNote note)
{
 // attach the pending dynamics if any to the note
  if (fPendingDynamicsList.size ()) {
    ...
    if (fCurrentNoteIsARest) {
      if (gGlobalMxmlTree2MsrOah->fDelayRestsDynamics) {
        fLogOutputStream <<
  ...
  S_msrNote note)
{
  ...
  if (fPendingWordsList.size ()) {
    ...
    if (fCurrentNoteIsARest) {
      if (gGlobalMxmlTree2MsrOah->fDelayRestsWords) {
        fLogOutputStream <<
  ...
  S_msrNote note)
{
  ...
  if (fPendingBeamsList.size ()) {
    ...
    if (fCurrentNoteIsARest) {
      if (gGlobalMxmlTree2MsrOah->fDelayRestsBeams) {
        fLogOutputStream <<
  ...
  S_msrNote note)
{
  ...
  if (fPendingSlursList.size ()) {
    ...
    if (fCurrentNoteIsARest) {
      if (gGlobalMxmlTree2MsrOah->fDelayRestsSlurs) {
        fLogOutputStream <<
  ...
  S_msrNote note)
{
  ...
  if (fPendingLigaturesList.size ()) {
    ...
    if (fCurrentNoteIsARest) {
      if (gGlobalMxmlTree2MsrOah->fDelayRestsLigatures) {
        fLogOutputStream <<
  ...
  S_msrNote note)
{
  ...
  if (fPendingPedalsList.size ()) {
    ...
    if (fCurrentNoteIsARest) {
      if (gGlobalMxmlTree2MsrOah->fDelayRestsPedals) {
        fLogOutputStream <<
  ...
  S_msrNote note)
{
  ...
  if (fPendingSlashesList.size ()) {
    ...
    if (fCurrentNoteIsARest) {
      if (gGlobalMxmlTree2MsrOah->fDelayRestsSlashes) {
        fLogOutputStream <<
  ...
  S_msrNote note)
{
  ...
  if (fPendingWedgesList.size ()) {
    ...
    if (fCurrentNoteIsARest) {
      if (gGlobalMxmlTree2MsrOah->fDelayRestsWedges) {
        fLogOutputStream <<
...
#ifdef TRACE_OAH
    if (gGlobalTraceOahGroup->fTraceNotes || gGlobalTraceOahGroup->fTraceStaffChanges) {
      fLogOutputStream <<
...
#ifdef TRACE_OAH
  if (gGlobalTraceOahGroup->fTraceChords) { // JMI
    fLogOutputStream <<
  ...
  */
  ...
  if (fCurrentNoteIsAGraceNote) {
    if (! fPendingGraceNotesGroup) {
      ...
      if (gGlobalMxmlTree2MsrOah->fSlurAllGraceNotes) {
        fPendingGraceNotesGroup->
...
#ifdef TRACE_OAH
          if (gGlobalTraceOahGroup->fTraceNotes || gGlobalTraceOahGroup->fTraceChords) {
            fLogOutputStream <<
...
#ifdef TRACE_OAH
    if (gGlobalTraceOahGroup->fTraceChords || gGlobalTraceOahGroup->fTraceTuplets) {
      fLogOutputStream <<
        "Adding chord '" <<
...
#ifdef TRACE_OAH
  if (gGlobalTraceOahGroup->fTraceChords || gGlobalTraceOahGroup->fTraceGraceNotes) {
    fLogOutputStream <<
...
#ifdef TRACE_OAH
  if (gGlobalTraceOahGroup->fTraceHarpPedalsTunings) {
    fLogOutputStream <<
...
/* JMI
#ifdef TRACE_OAH
  if (gGlobalTraceOahGroup->fTraceNotes) { // JMI
