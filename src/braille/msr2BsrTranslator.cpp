...
void msr2BsrTranslator::visitStart (S_msrPageBreak& elt)
{
  ...
  fCurrentPage =
    bsrPage::create (
      ...
      gGlobalBsr2BrailleOah->fLinesPerPage);
...
/*
  ...
  if (gGlobalLpsr2LilypondOah->fPoet.size ()) {
    // define poet
...
#ifdef TRACE_OAH
  if (gGlobalTraceOahGroup->fTraceParts || gGlobalTraceOahGroup->fTraceMeasures) {
    fLogOutputStream <<
      endl <<
...
void msr2BsrTranslator::visitStart (S_msrClef& elt)
{
  ...
  if (gGlobalMsr2BsrOah->fIncludeClefs) {
...
void msr2BsrTranslator::visitStart (S_msrSyllable& elt)
{
  ...
  if (fOnGoingStanza) { // fCurrentStanzaClone JM
    ...
    if (gGlobalBsrOah->fAddWordsFromTheLyrics) {
      ...
      if (syllableTextsList.size ()) {
        ...
        // create the words
#ifdef TRACE_OAH
        if (gGlobalTraceOahGroup->fTraceLyrics || gGlobalTraceOahGroup->fTraceWords) {
          fLogOutputStream <<
            "Changing lyrics '" <<
            wordsValue <<
            "' into words for note '" <<
...
void msr2BsrTranslator::visitStart (S_msrGraceNotesGroup& elt)
{
...
#ifdef TRACE_OAH
    if (gGlobalTraceOahGroup->fTraceNotes || gGlobalTraceOahGroup->fTraceGraceNotes) {
      fLogOutputStream <<
  ...
  if (
    ...
    gGlobalTraceOahGroup->fTraceNotes
      ||
    gGlobalTraceOahGroup->fTraceVoices
  ) {
    fLogOutputStream <<
...
#ifdef TRACE_OAH
      if (gGlobalTraceOahGroup->fTraceGraceNotesGroup) {
        fLogOutputStream <<
...
#ifdef TRACE_OAH
        if (gGlobalTraceOahGroup->fTraceGraceNotes || gGlobalTraceOahGroup->fTraceNotes) {
          fLogOutputStream <<
...
void msr2BsrTranslator::visitStart (S_msrRepeatCommonPart& elt)
{
  ...
  if (
    gGlobalTraceOahGroup->fTraceRepeats
      ||
    gGlobalTraceOahGroup->fTraceVoicesDetails
  ) {
    gLogOstream <<
      endl <<
...
void msr2BsrTranslator::visitEnd (S_msrRepeatEnding& elt)
{
  ...
  if (gGlobalTraceOahGroup->fTraceRepeats || gGlobalTraceOahGroup->fTraceVoices) {
    gLogOstream <<
      endl <<
      "*********>> msrRepeatEnding HHH " <<
      ", line " << inputLineNumber <<
