...
\tikzmath{
  ...
  \xmlToLilyPondInterLength = 0.2;
  \xmlToLilyPondStartAbs = \MusicXMLAbs + 0.05;
  \xmlToLilyPondStartOrd = \MusicXMLOrd - 0.15;
  \xmlToLilyPondInterIAbs = cos(\xmlToLilyPondInterIAngle) * \xmlToLilyPondInterLength;
  \xmlToLilyPondInterIOrd = sin(\xmlToLilyPondInterIAngle) * \xmlToLilyPondInterLength;
  \xmlToLilyPondInterIIAbs = cos(\xmlToLilyPondInterIIAngle) * \xmlToLilyPondInterLength;
  \xmlToLilyPondInterIIOrd = sin(\xmlToLilyPondInterIIAngle) * \xmlToLilyPondInterLength;
...
\node[right,scale=2] (title) at (\titleAbs,\titleOrd) {libmusicxml:};
...
LPSR & LilyPond Score Representation, i.e. MSR plus data representing the {\tt $\backslash$score} block
\tabularnewline
...
tools & a set of other demo programs such as countnotes and partsummary, showing the use of the library
\tabularnewline

xml2lilypond & performs the 4 hops from MusicXML to LilyPond
\tabularnewline
...
%\node[right,scale=1] (comment) at (\legendAbs,\bottomLineOrd+0.7) {~xmlelement tree: a tree representing the MusicXML markups such as {\tt <part-list>}, {\tt <time>} and {\tt <note>}};

%\node[right,scale=1] (comment) at (\legendAbs,\bottomLineOrd+0.6) {~MSR: Music Score Representation, in terms of part groups, parts, staves, voices, notes, etc};

%\node[right,scale=1] (comment) at (\legendAbs,\bottomLineOrd+0.5) {~LPSR: LilyPond Score Representation, i.e. MSR plus data representing the {\tt $\backslash$score} block};

%\node[right,scale=1] (comment) at (\legendAbs,\bottomLineOrd+0.4) {~MDSR: MIDI Score Representation, to be designed};

% Draw the legend
% ------------------------------------------------

%\node[right,scale=1] (comment) at (\legendAbs,\bottomLineOrd+0.3) {$\bullet$ RandomMusic generates an xmlelement tree containing random music and writes it as MusicXML};

%\node[right,scale=1] (comment) at (\legendAbs,\bottomLineOrd+0.2) {$\bullet$ countnotes and partsummary are other sample programs showing the use of the library};

%\node[right,scale=1] (comment) at (\legendAbs,\bottomLineOrd+0.1) {$\bullet$ xml2lilypond performs the 4 hops from MusicXML to LilyPond};

\node[right,scale=1] (comment) at (\legendAbs,\bottomLineOrd+0.0) {$\bullet$ note: LilyPond can generate MIDI files from its input};


% Draw the date
% ------------------------------------------------

\node[right,scale=1] (comment) at (\dateAbs,\bottomLineOrd) {v2.2/0.1.0, February 2017};
...
\filldraw [gray!30] 
  ...
  ..
  node[above,pos=0.8,sloped] {tools}
...
%\node[align=left] (trace) at (-1, 2.5) {
%  xmlToLilyPondInterIAbs = \xmlToLilyPondInterIAbs~
%  xmlToLilyPondInterIOrd = \xmlToLilyPondInterIOrd~
%  
%  xmlToLilyPondInterIIAbs = \xmlToLilyPondInterIIAbs~
%  xmlToLilyPondInterIIOrd = \xmlToLilyPondInterIIOrd
%};
...
\filldraw [gray!30] 
  ...
  (\xmlToLilyPondEndAbs,\xmlToLilyPondEndOrd) circle [radius=1pt];
