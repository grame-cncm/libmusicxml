...
void msr2BsrTranslator::visitStart (S_msrPageBreak& elt)
{
  ...
  fCurrentPage =
    bsrPage::create (
      ...
      globalBsr2BrailleOah->fLinesPerPage);
...
/*
  ...
  if (globalLpsr2LilypondOah->fPoet.size ()) {
    // define poet
...
#ifdef TRACE_OAH
  if (globalTraceOah->fTraceParts || globalTraceOah->fTraceMeasures) {
    fLogOutputStream <<
      endl <<
...
void msr2BsrTranslator::visitStart (S_msrClef& elt)
{
  ...
  if (globalMsr2BsrOah->fIncludeClefs) {
...
void msr2BsrTranslator::visitStart (S_msrSyllable& elt)
{
  ...
  if (fOnGoingStanza) { // fCurrentStanzaClone JM
    ...
    if (globalBsrOah->fAddWordsFromTheLyrics) {
      ...
      if (syllableTextsList.size ()) {
        ...
        // create the words
#ifdef TRACE_OAH
        if (globalTraceOah->fTraceLyrics || globalTraceOah->fTraceWords) {
          fLogOutputStream <<
            "Changing lyrics '" <<
            wordsValue <<
            "' into words for note '" <<
...
#ifdef TRACE_OAH
    if (globalTraceOah->fTraceNotes || globalTraceOah->fTraceGraceNotes) {
      fLogOutputStream <<
...
#ifdef TRACE_OAH
      if (globalTraceOah->fTraceGraceNotesGroup) {
        fLogOutputStream <<
...
#ifdef TRACE_OAH
        if (globalTraceOah->fTraceGraceNotes || globalTraceOah->fTraceNotes) {
          fLogOutputStream <<
...
void msr2BsrTranslator::visitStart (S_msrRepeatCommonPart& elt)
{
  ...
  if (
    globalTraceOah->fTraceRepeats
      ||
    globalTraceOah->fTraceVoicesDetails
  ) {
    gLogOstream <<
      endl <<
...
void msr2BsrTranslator::visitEnd (S_msrRepeatEnding& elt)
{
  ...
  if (globalTraceOah->fTraceRepeats || globalTraceOah->fTraceVoices) {
    gLogOstream <<
      endl <<
      "*********>> msrRepeatEnding HHH " <<
      ", line " << inputLineNumber <<
