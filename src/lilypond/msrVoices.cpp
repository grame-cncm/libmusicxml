  ...
  string context)
{
  ...
  if (
    ...
    gGeneralOptions->fTraceRepeats
      ||
    gGeneralOptions->fTraceMeasureRepeats
      ||
...
S_msrMeasure msrVoice::createMeasureSecondPart (
  int    inputLineNumber,
  string measureNumber,
  int    measureOrdinalNumber,
  msrMeasure::msrMeasureImplicitKind
...
void msrVoice::displayVoiceRepeatsStack (
  ...
  string context)
{
  int repeatDescrsStackSize = fVoiceRepeatDescrsStack.size ();
  ...
  if (repeatDescrsStackSize) {  
    list<S_msrRepeatDescr>::const_iterator
      iBegin = fVoiceRepeatDescrsStack.begin (),
      iEnd   = fVoiceRepeatDescrsStack.end (),
      i      = iBegin;
    ...
    case msrMeasure::kUnderfullMeasureKind:
      {
        ...
        // create a new measure with the same number as measure
        result =
          measure->
            createMeasureNewbornClone (
              measure->getMeasureSegmentUplink ());
   //         msrMeasure::kMeasureImplicitYes); // JMI
  ...
  string      context)
{
  ...
  fVoiceRepeatDescrsStack.push_front (
  ...
  string      context)
{
  ...
  if (repeat != fVoiceRepeatDescrsStack.front ()->getRepeatDescrRepeat ()) {
    stringstream s;
  ...
  fVoiceRepeatDescrsStack.pop_front ();
    ...
    case msrVoice::kFiguredBassVoice:
      {
        ...
        if (
          ...
          gLogIOstream <<
            ...
            fVoiceName <<
            "\" after createMeasuresRepeatFromItsFirstMeasuresInVoice () is:" <<
            endl;
...
/* JMI
        // create the rest measures segment
        S_msrSegment
          restsSegment =
            msrSegment::create (
              inputLineNumber,
    ...
    case msrVoice::kFiguredBassVoice:
      {
        ...
        // grab the just created last measure in the last segment's measure list,
        // which is the next measure after the rest measures
        if (! voiceLastSegmentMeasureList.size ()) {
          stringstream s;
  ...
  int inputLineNumber)
{
  ...
  // create fCurrentVoiceRestMeasures' rest contents
#ifdef TRACE_OPTIONS
  if (gGeneralOptions->fTraceRestMeasures) {
    gLogIOstream <<
...
#ifdef TRACE_OPTIONS
        if (gGeneralOptions->fTraceMeasureRepeats) { // JMI
          gLogIOstream <<
            endl <<
            "***********" <<
            endl <<
...
{
#ifdef TRACE_OPTIONS
  if (gGeneralOptions->fTraceMeasureRepeats || gGeneralOptions->fTraceSegments) {
    gLogIOstream <<
      "Appending measures repeat '" <<
