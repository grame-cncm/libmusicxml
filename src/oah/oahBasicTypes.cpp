...
/* JMI
void oahHandler::checkOptionsAndArgumentsFromArgcAndArgv (
  vector<string>& theArgumentsVector)
{
#ifdef TRACING_IS_ENABLED
  if (gGlobalTraceOahGroup->getTraceOah ()) {
    string separator =
    ...
    gLogStream <<
      ...
      gTab <<
      fHandlerExecutableName <<
      "checking the options and arguments from argc/argv in \"" <<
      fHandlerHeader <<
      "\"" <<
      endl;
  ...
  oahInternalError (
    "the options vector  with argc/argv should be checked in subclasses");
}
*/
  ...
  string        subGroupName)
{
  ...
  if (gGlobalTraceOahGroup->getTraceOah ()) {
    gLogStream <<
      "Handling option name \"" << subGroupName << "\" of type 'subGroup'" <<
      endl;
  }
#endif
