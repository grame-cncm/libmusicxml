...
{
  if (gGlobalLpsr2LilypondOah->fRelativeOctaveEntrySemiTonesPitchAndOctave) {
    // option '-rel, -relative' has been used:
    fCurrentOctaveEntryReference =
      msrNote::createNoteFromSemiTonesPitchAndOctave (
        K_NO_INPUT_LINE_NUMBER,
        gGlobalLpsr2LilypondOah->fRelativeOctaveEntrySemiTonesPitchAndOctave);
  }
  else {
  ...
  if (gGlobalTraceOahGroup->fTraceNotesOctaveEntry) {
    fLogOutputStream <<
      ...
      ", octaveEntryKind is" <<
      lpsrOctaveEntryKindAsString (gGlobalLpsr2LilypondOah->fOctaveEntryKind) <<
      endl <<
...
*/
  ...
  /* initialize reference is:
        ...
        fixed  in fixed mode
  */
  switch (gGlobalLpsr2LilypondOah->fOctaveEntryKind) {
    ...
    case kOctaveEntryFixed:
      // sanity check
      msgAssert (
        gGlobalLpsr2LilypondOah->fFixedOctaveEntrySemiTonesPitchAndOctave != nullptr,
        "gGlobalLpsr2LilypondOah->fFixedOctaveEntrySemiTonesPitchAndOctave is null");

      fCurrentOctaveEntryReference =
        msrNote::createNoteFromSemiTonesPitchAndOctave (
          K_NO_INPUT_LINE_NUMBER,
          gGlobalLpsr2LilypondOah->fFixedOctaveEntrySemiTonesPitchAndOctave);
      break;
  } // switch
  ...
  S_msrNote note)
{
  ...
  string
    quarterTonesPitchKindAsString =
      msrQuarterTonesPitchKindAsString (
        gGlobalLpsrOah->fLpsrQuarterTonesPitchesLanguageKind,
        noteQuarterTonesPitchKind);
  ...
  rational wholeNotes)
{
  ...
  if (wholeNotes != fLastMetWholeNotes) {
    ...
    generateExplicitDuration =
      gGlobalLpsr2LilypondOah->fAllDurations;
  }
...
//  if (gGlobalLpsr2LilypondOah->) { // JMI
...
/* JMI BOF
            if (fOnGoingVoiceCadenza) { // JMI
              if (noteSoundingWholeNotes != rational (1, 1)) {
                ...
                if (! gGlobalLpsr2LilypondOah->fAllDurations) {
    ...
    case msrNote::kSkipNote:
      if (gGlobalLpsr2LilypondOah->fPositionsInMeasures) {
    ...
    case msrNote::kUnpitchedNote:
      {
        ...
        if (note->getNoteDelayedTurnOrnament ()) {
          ...
          // regardless of gGlobalGeneralOahGroup->fAllDurations
          fLilypondCodeOstream <<
            wholeNotesAsLilypondString (
              inputLineNumber,
              noteSoundingWholeNotes) <<
            "*" <<
            gGlobalLpsr2LilypondOah->fDelayedOrnamentsFraction;
        }
...
*/
    ...
    case msrNote::kTupletMemberNote:
      if (gGlobalLpsr2LilypondOah->fIndentTuplets) {
        fLilypondCodeOstream << endl;
      }
  ...
  S_msrMeasureElement measureElement)
{
  ...
  if (gGlobalLpsr2LilypondOah->fInputLineNumbers) {
  ...
  markupColumnKind columnKind)
{
  ...
  if (chunksList.size ()) {
    ...
    if (gGlobalLpsr2LilypondOah->fLilyPondComments) {
      s <<
        "% " <<
        singularOrPlural (
...
void lpsr2LilypondTranslator::visitStart (S_lpsrHeader& elt)
{
  ...
  {
    // title
    if (gGlobalMxmlTree2MsrOah->fUseFilenameAsWorkTitle) {
      fLilypondCodeOstream << left <<
        setw (fieldNameWidth) <<
        "title = \"" <<
        gGlobalOahOahGroup->fInputSourceName <<
...
//  if (gGlobalLpsrOah->fPaperHeight.getLengthValue () >= 0.0) { // JMI BLARK
  if (gGlobalLpsrOah->fPaperHeightAtom->getVariableHasBeenSet ()) {
    paperHeight =
      gGlobalLpsrOah->fPaperHeight;
  }
  else if (pageLayout) {
    paperHeight =
...
//  if (gGlobalLpsrOah->fPaperWidth.getLengthValue () >= 0.0) { // JMI BLARK
  if (gGlobalLpsrOah->fPaperWidthAtom->getVariableHasBeenSet ()) {
    paperWidth =
      gGlobalLpsrOah->fPaperWidth;
  }
  else if (pageLayout) {
    paperWidth =
...
void lpsr2LilypondTranslator::visitStart (S_lpsrLayout& elt)
{
  ...
  if (gGlobalLpsr2LilypondOah->fAmbitusEngraver) {
    fLilypondCodeOstream <<
        gTab << "\\consists \"Ambitus_engraver\"" <<
        endl;
  }

  if (gGlobalLpsr2LilypondOah->fCustosEngraver) {
    fLilypondCodeOstream <<
...
void lpsr2LilypondTranslator::visitEnd (S_lpsrLayout& elt)
{
  ...
  if (gGlobalLpsr2LilypondOah->fRepeatBrackets) {
    fLilypondCodeOstream <<
      "\\context " "{" <<
      endl;
...
// JMI  fLilypondCodeOstream << endl << endl << partGroup << endl << endl;
  ...
  if (gGlobalLpsr2LilypondOah->fConnectArpeggios) {
    fLilypondCodeOstream <<
...
void lpsr2LilypondTranslator::visitStart (S_lpsrUseVoiceCommand& elt) // JMI ???
{
 ...
 // if (voice->getStaffRelativeVoiceNumber () > 0) { JMI
    ...
    if (gGlobalLpsr2LilypondOah->fNoAutoBeaming) {
      fLilypondCodeOstream <<
...
/* JMI
    ...
    if (partName.size ()) { // a part name is not mandatory in MusicXML
      // check by name
      if (gGlobalLpsr2LilypondOah->fPartNamesTranspositionMap.size ()) {
        map<string, S_msrSemiTonesPitchAndOctave>::const_iterator
          it =
            gGlobalLpsr2LilypondOah->fPartNamesTranspositionMap.find (
              partName);

        if (it != gGlobalLpsr2LilypondOah->fPartNamesTranspositionMap.end ()) {
    ...
    if (gGlobalLpsr2LilypondOah->fPartIDsTranspositionMap.size ()) {
      map<string, S_msrSemiTonesPitchAndOctave>::const_iterator
        it =
          gGlobalLpsr2LilypondOah->fPartIDsTranspositionMap.find (
            partID);

      if (it != gGlobalLpsr2LilypondOah->fPartIDsTranspositionMap.end ()) {
...
void lpsr2LilypondTranslator::visitStart (S_lpsrNewLyricsBlock& elt)
{
  ...
  if (! gGlobalLpsr2LilypondOah->fNoLilypondLyrics) {
    ...
    if (gGlobalLpsr2LilypondOah->fAddStanzasNumbers) {
      fLilypondCodeOstream <<
        gTab << "stanza = \"" <<
    ...
    switch (gGlobalLpsr2LilypondOah->fLyricsDurationsKind) {
    ...
    for ( ; ; ) {
      fLilypondCodeOstream <<
        msrQuarterTonesPitchAsString (
          gGlobalLpsrOah->fLpsrQuarterTonesPitchesLanguage,
      ...
      switch (gGlobalLpsr2LilypondOah->fOctaveEntryKind) {
        case kOctaveEntryRelative:
          ...
          if (
            gGlobalLpsr2LilypondOah->fRelativeOctaveEntrySemiTonesPitchAndOctave
//      JMI         !=
//            gGlobalLpsr2LilypondOah->fSemiTonesPitchAndOctaveDefaultValue
          ) {
            // option '-rel, -relative' has been used
            fLilypondCodeOstream <<
              " " <<
  ...
  // \displayMusic?
  if (gGlobalLpsr2LilypondOah->fDisplayMusic) {
    fLilypondCodeOstream <<
      "\\displayMusic {" <<
      endl;
  ...
  // chords language
  if (gGlobalLpsrOah->fLpsrChordsLanguageKind != k_IgnatzekChords) {
    fLilypondCodeOstream <<
      "\\" <<
  ...
  if (gGlobalLpsr2LilypondOah->fShowAllBarNumbers) {
    fLilypondCodeOstream <<
...
/* JMI
  if (
    fCurrentVoice->getVoiceContainsRestMeasures ()
      ||
    gGlobalLpsr2LilypondOah->fCompressFullMeasureRests
  ) {
    fLilypondCodeOstream <<
      "\\compressFullBarRests" <<
      endl;
...
*/
  ...
  if (gGlobalLpsr2LilypondOah->fAccidentalStyleKind != kDefault) {
    fLilypondCodeOstream <<
...
void lpsr2LilypondTranslator::visitStart (S_msrVoiceStaffChange& elt)
{
  ...
  if (
    gGlobalLpsr2LilypondOah->fInputLineNumbers
      ||
    gGlobalLpsr2LilypondOah->fPositionsInMeasures
  ) {
...
void lpsr2LilypondTranslator::visitStart (S_msrMeasure& elt)
{
  ...
  // should we generate a box around this bar number?
  {
    ...
    if (it != gGlobalLpsr2LilypondOah->fBoxAroundBarNumberSet.end ()) {
  ...
  // should we reset the measure purist number?
  {
    ...
    if (it != gGlobalLpsr2LilypondOah->fResetMeasureElementMeasureNumberMap.end ()) {
...
if (false) // JMI
    ...
    */
          ...
          if (gGlobalLpsr2LilypondOah->fBreakLinesAtIncompleteRightMeasures) {
            fLilypondCodeOstream <<
              "\\break" <<
              endl;
          }
...
void lpsr2LilypondTranslator::visitEnd (S_msrMeasure& elt)
{
  ...
  if (! fOnGoingRestMeasures) {
    ...
    if (gGlobalLpsr2LilypondOah->fSeparatorLineEveryNMeasures > 0) {
      if (
        fCurrentVoiceMeasuresCounter
          %
        gGlobalLpsr2LilypondOah->fSeparatorLineEveryNMeasures
          ==
        0)
        fLilypondCodeOstream <<
          endl <<
  ...
  // should we generate a line break?
  {
    ...
    if (it != gGlobalLpsr2LilypondOah->fBreakLineAfterMeasureNumberSet.end ()) {
  ...
  #ifdef TRACE_OAH
     if (gGlobalTraceOahGroup->fTraceLineBreaks) { // JMI
        fLogOutputStream <<
          endl <<
          "Measure number '" <<
          measureNumber <<
          "' not found in gGlobalLpsr2LilypondOah->fBreakLineAfterMeasureNumberSet" <<
          ", line " << inputLineNumber <<
          endl;
      }
  #endif
  ...
  // should we generate a page break?
  {
    ...
    if (it != gGlobalLpsr2LilypondOah->fBreakPageAfterMeasureNumberSet.end ()) {
  ...
  #ifdef TRACE_OAH
     if (gGlobalTraceOahGroup->fTracePageBreaks) { // JMI
        fLogOutputStream <<
          endl <<
          "Measure number '" <<
          measureNumber <<
          "' not found in gGlobalLpsr2LilypondOah->fBreakPageAfterMeasureNumberSet" <<
          ", line " << inputLineNumber <<
          endl;
      }
  #endif
...
void lpsr2LilypondTranslator::visitStart (S_msrClef& elt)
{
  ...
  if (gGlobalLpsr2LilypondOah->fCommentClefChanges) {
  /* JMI
    S_msrClef
  ...
  if (clefKind != k_NoClef) {
    ...
    switch (clefKind) {
      ...
      case kTablature7Clef:
        if (gGlobalLpsr2LilypondOah->fModernTab) {
          fLilypondCodeOstream <<
            "\"moderntab\"" <<
            endl;
        }
        ...
        if (gGlobalLpsr2LilypondOah->fTabFullNotation) {
          fLilypondCodeOstream <<
            "\\tabFullNotation" <<
            endl;
        }
      ...
      switch (beam->getBeamKind ()) {
        ...
        case msrBeam::kBeginBeam:
          if (beam->getBeamNumber () == 1) {
            if (! gGlobalLpsr2LilypondOah->fNoBeams) {
 #ifdef TRACE_OAH
              if (gGlobalTraceOahGroup->fTraceBeams) {
                gLogOstream <<
        ...
        // don't handle rest measures, that's done in visitEnd (S_msrRestMeasures&)
          ...
          if (elt->getNoteOccupiesAFullMeasure ()) {
            ...
            if (inhibitRestMeasuresBrowsing) {
              if (
                gGlobalMsrOah->fTraceMsrVisitors
                  ||
                gGlobalTraceOahGroup->ffTraceRestMeasures) {
                gLogOstream <<
    ...
    if (gGlobalTraceOahGroup->fTraceNotesDetails) {
      ...
      endl;
          ...
          */
          ...
          if (
            gGlobalMsrOah->fTraceMsrVisitors
              ||
            gGlobalTraceOahGroup->fTraceRestMeasures
          ) {
            gLogOstream <<
      ...
      case msrNote::kGraceSkipNote:
          ...
          if (
            gGlobalMsrOah->fTraceMsrVisitors
              ||
            gGlobalTraceOahGroup->fTraceGraceNotes
          ) {
            gLogOstream <<
...
*/
    ...
    case msrBarline::kBarlineCategoryRepeatStart:
      if (gGlobalLpsr2LilypondOah->fKeepRepeatBarlines) {
      /*
        if (gGlobalLpsr2LilypondOah->fRepeatBrackets) {
...
void lpsr2LilypondTranslator::visitStart (S_msrBarCheck& elt)
{
  ...
  if (
    ...
    if (gGlobalLpsr2LilypondOah->fOriginalMeasureNumbers) {
...
void lpsr2LilypondTranslator::visitStart (S_msrMidiTempo& elt)
{
  ...
  if (gGlobalLpsr2LilypondOah->fNoMidi) {
    fLilypondCodeOstream <<
      "%{" <<
      endl;
