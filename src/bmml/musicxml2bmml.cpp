...
static xmlErr xml2bmml (SXMLFile& xmlfile, const optionsVector& options, std::ostream& out, std::ostream& err, const char* file)
{
	Sxmlelement st;
...
*/
    ...
    // create BMML from the MSR
    // ------------------------------------------------------
    try {
      convertMsrScoreToBMMLScore (
        mScore,
        regex_replace (
          file,
          regex (".ly"),
    ...
    catch (msrScoreToBMMLScoreException& e) {
      return kInvalidFile;
    }
    catch (std::exception& e) {
      return kInvalidFile;
...
EXP xmlErr musicxmlfile2bmml (const char *file, const optionsVector& options, std::ostream& out, std::ostream& err)
{
	...
	if (xmlfile) {
		return xml2bmml(xmlfile, options, out, err, file);
	}
...
EXP xmlErr musicxmlfd2bmml (FILE * fd, const optionsVector& options, std::ostream& out, std::ostream& err)
{
	...
	if (xmlfile) {
		return xml2bmml(xmlfile, options, out, err, 0);
	}
...
//_______________________________________________________________________________
EXP xmlErr convertMsrScoreToBMMLScore (
  S_msrScore                 mScore,
...
//_______________________________________________________________________________
EXP xmlErr convertMusicXMLToBMML (
  ...
  string outputFileName)
{
  ...
  // create BMML from the MSR (pass 3)
  // ------------------------------------------------------
