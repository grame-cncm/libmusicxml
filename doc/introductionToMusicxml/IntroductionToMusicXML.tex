...
In the DTD, '{\tt CDATA}' means {\it Character Data}. In the same vein, '{\tt PCDATA}' means {\it Parsed Character Data}, i.e. mixed content XML elements.  
...
\begin{itemize}
\item an optional '{\tt <work>}' element, containing elements such as '{\tt <work-number>}', '{\tt <work-title>}' and '{\tt <opus>}';

\item optional '{\tt <movement-number>}', '{\tt <movement-title>}', '{\tt <identification>}' and '{\tt <defaults>}' elements;
...
This is how the score header is defined in \schemafile{score.mod}:
\begin{lstlisting}[language=XML]
<!--
...
In \mxmlfile{harmonies/Inversion.xml}, shown below, there is a harmony with an '{\tt <inversion>}' element containing '{\tt 2}', hence the second inversion of the chord. A number of applications ignore this element when importing \mxml\ data, because it takes a full knowledge of chords structures to compute the bass note of inverted chords.
...
All the other applications s in this panel produce the following, i.e. they ignore the beat repeat altogether:\\
\includegraphics{BeatRepeatXml2ly.png}

And if one exports the score from \fin\ to \mxml, the beat repeat information is lost, see \mxmlfile{repeats/BeatRepeatExportedFromFinale.xml}. 

All this seems to indicate that beat repeats are quite rare in \mxml\ files.
