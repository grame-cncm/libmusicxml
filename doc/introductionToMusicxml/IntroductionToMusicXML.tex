...
\item the '{\tt <part>}' element has an '{\tt id}' attribute containing the name of the part;
\item the '{\tt <measure>}' element's attributes contain the measure number and its width.
\end{itemize}

Note that some elements contain unnamed data such as durations and voice and staff numbers:
\begin{lstlisting}[language=XML]
        <duration>4</duration>
        <voice>2</voice>
        <staff>1</staff>
...
In a DTD, '{\tt CDATA}' means {\it Character Data}. Such data is not analyzed by the software  the reads the \mxml\ data, it is merely passed over to whoever asked the data to be read in.

In the same vein, '{\tt PCDATA}' means {\it Parsed Character Data}, that is, mixed content XML data that are analyzed.  
...
\mxml\ data is made of so-called markups (the 'M' in 'XML'), delimited by an opener and a closer. The opener is introduced by a '{\tt <}' and closed by a '{\tt >}', as in '{\tt <part-list>}'. The closer is introduced by a '{\tt </}' and closed by a '{\tt >}', as in '{\tt </part-list>}'.
...
Some elements in \mxml\ data are specifically meant for MIDI support.
...
Measure can be much longer that the usual time signagures, see section \ref{lyrics}.
...
The '{\tt <syllabic>}' sub-element, contains a characterization of the text:
%\begin{adjustwidth}{-0.5cm}{-0.5cm}
\begin{center}
\footnotesize
...
%
\begin{tabular}[t]{lp{\contentsWidth}}
{\bf value}&{\bf Meaning} \tabularnewline[0.5ex]
\hline\\[-3.0ex]
%

'{\tt single}' & the text is a standalone word
\tabularnewline
...
Multi-measure rests are specified in \mxml\ with the '{\tt <multiple-rest>}' element.
For example, \mxmlfile{rests/MultiMeasureRests.xml} contains:
\begin{lstlisting}[language=XML]
      <attributes>
        <!-- ... ... ... ... -->
        <measure-style>
...
In the snippet above, the current measure is the first of a sequence of 2 rest measures.
Note that all measures in the sequence have to be explicitly present in the \mxml\ data.
