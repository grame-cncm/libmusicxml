  ...
  string                context)
{
  ...
  if (gGeneralOptions->fTraceRepeats) {
    gLogIOstream <<
      "Moving the voice last segment to repeat common part '" <<
      repeatCommonPart->asShortString () <<
      "' in voice \"" <<
      getVoiceName () <<
      "\"" <<
...
void msrVoice::appendSegmentCloneToInitialVoiceElements (
  int          inputLineNumber,
  ...
  int inputLineNumber)
{
    ...
    case msrVoice::kFiguredBassVoice:
      ...
      if (fVoiceLastSegment) {
        ...
        if (fVoiceLastSegment->getSegmentMeasuresList ().size ()) {
          ...
          appendSegmentCloneToInitialVoiceElements (
            inputLineNumber,
            fVoiceLastSegment,
            "");
  ...
  */
          ...
          if (
            ...
            gLogIOstream <<
              endl <<
              "*********>> Current voice nestContentsIntoNewRepeatInVoice() 2 \"" <<
              getVoiceName () <<
              "\"" <<
              ", line " << inputLineNumber <<
              " contains:" <<
...
// JMI     // append a new measure with the same number as the voice last measure
    // to the voice,
      ...
      if (fVoiceLastSegment) {
        ...
        if (voiceLastSegmentMeasuresList.size ()) {
          ...
          if (
            ...
            gLogIOstream <<
              endl <<
              "*********>> Current voice handleRepeatStartInVoice() 1 \"" <<
              getVoiceName () <<
              "\"" <<
              ", line " << inputLineNumber <<
              " contains:" <<
            ...
            case msrMeasure::kEmptyMeasureKind:
              {
                ...
                // keep it and leave the voice last segment as it is
      ...
      // create the repeat and stack it
        ...
        S_msrRepeat
          ...
          if (
            ...
            gLogIOstream <<
              endl <<
              "*********>> Current voice handleRepeatStartInVoice() 2 \"" <<
              getVoiceName () <<
              "\"" <<
              ", line " << inputLineNumber <<
              " contains:" <<
  ...
  int    repeatTimes)
{
  ...
  if (gGeneralOptions->fTraceRepeats) {
    gLogIOstream <<
      "Time for measure '" <<
      previousLastSegmentLastMeasure->asString () <<
      "' is '" <<
      time->asString () <<
      "'" <<
      endl;
    ...
    previousLastSegmentLastMeasure->print (gLogIOstream);
...
// JMI     // append a new measure with the same number as the voice last measure
    ...
    createMeasureAndAppendItToVoice (
      inputLineNumber,
      measureNumber,
      998, // measure ordinal number JMI
      msrMeasure::kMeasureImplicitNo);
          ...
          case 0:
            {
              ...
              if (
                ...
                gLogIOstream <<
                  endl <<
                  "*********>> Current voice handleRepeatEndingStartInVoice \"" <<
                  getVoiceName () <<
                  "\"" <<
                  ", line " << inputLineNumber <<
                  " contains:" <<
    ...
    case msrVoice::kFiguredBassVoice:
      {
        ...
        if (
          ...
          gLogIOstream <<
            endl <<
            "*********>> Current voice finalizeRepeatEndInVoice() 1 \"" <<
            getVoiceName () <<
            "\"" <<
            ", line " << inputLineNumber <<
            " contains:" <<
...
/* NO JMI
        ...
        if (
          ...
          gLogIOstream <<
            endl <<
            "*********>> Current voice finalizeRepeatEndInVoice() 2 \"" <<
            getVoiceName () <<
            "\"" <<
            ", line " << inputLineNumber <<
            " contains:" <<
...
/* JMI
        ...
        finalizeCurrentMeasureInVoice (
          ...
          */
              ...
              if (gGeneralOptions->fTraceRepeats) {
                gLogIOstream <<
                  "Appending the voice last segment to the repeat common part in voice \"" <<
                  getVoiceName () <<
                  "\"" <<
                  ", line " << inputLineNumber <<
                  endl;
