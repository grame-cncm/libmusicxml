...
\mxml\ ({\it Music eXtended Markup Language}, \url{https://www.musicxml.com}) is a specification language meant to represent western music scores by texts, readable both by humans and computers, and to help sharing music score files between applications, through export and import mechanisms.. 
...
\hand {\mxml\ is a representation of HOW TO DRAW a score, which has implications on the kind of markups available, in particular '{\tt <forward>}' and '{\tt <backup>}', which are presented at \mylink{forwardAndBackup}.
}
...
Anacruses are best specified "the purist way", with '{\tt 0}' as their number and the '{\tt implicit}' attribute set to '{\tt yes}', which specifies that this measure number should not be printed. One see cases where the number is 1 for anacruses, though:
\begin{lstlisting}[language=XML]
...
Measure can be much longer that the usual time signatures, see \mylink{lyrics}, for an example.
...
The '{\tt <credit>}' element, defined in \schemafile{score.mod}, represents various legal informations about the score. It contains placement indication such as page number and alignment.
...
\begin{itemize}
\item \sib\ loses the last group, i.e. the bassoons;
\item \fin\ loses them too;
\item \muse\ crashes;
...
% -------------------------------------------------------------------------
\subsection{'{\tt <forward>}' and '{\tt <backup>}'}
\mylabel{forwardAndBackup}
% -------------------------------------------------------------------------
...
These two types of curves look similar graphically, but are quite different musically:
\begin{itemize}
\item a tie occur between two notes either same pitched or enharmonically equivalent, to indicate that the first one is to be maintained, without stopping the sound emission, as long as the second one. Several ties can appear in sequence on the same note. This is also a way to express arbitrarily long notes across measures;
\item a slur means that notes of various pitches and durations are to be played without interrupting the sound.
\end{itemize}

Before looking at an example, let's see how \mxml\ supports these two concepts.

% -------------------------------------------------------------------------
\subsection{Ties}
% -------------------------------------------------------------------------

\mxml\ makes the distinction between graphics and sound, and this applied to ties: the '{\tt <tie>}' element indicates sound, and the '{\tt <tied>}'	element indicates notation.
...
In \mxmlfile{harmonies/Inversion.xml}, shown below, there is a harmony with an '{\tt <inversion>}' element containing '{\tt 2}', hence the second inversion of the chord should be displayed. A number of applications ignore this element when importing \mxml\ data, because it takes a full knowledge of chords structures to compute the bass note of inverted chords.
...
\mxmlToLy\ is still experimental, and currently produces:\\
...
It seems thus that beat repeats are quite rare in \mxml\ files.
