...
#ifndef __xmlpart2lilypond__
#define __xmlpart2lilypond__
  ...
  public visitor<S_wedge>
{
  ...
  std::list<Slilypondwedge>  fPendingWedges;
 
  // fields to controls the lilypond output generation
  ...
  void startPartStack (Slilypondelement& elt)    { fPartStack.push(elt); }
  void addToPartStack  (Slilypondelement& elt)   { fPartStack.top()->add(elt); }
  void addDelayed (Slilypondelement elt, long offset);  // adding elements to the delayed elements
  void checkDelayed (long time);            // checks the delayed elements for ready elements 
  void pushToPartStack (Slilypondelement& elt)   { addToPartStack(elt); fPartStack.push(elt); }
  void popFromPartStack ()             { fPartStack.pop(); }
  ...
  lilypondnoteduration  noteDuration  ( const notevisitor& nv );
  ...
  protected:
    ...
    virtual void visitEnd  ( S_wedge& elt);
  ...
  public:
    ...
    xmlpart2lilypond( translationSwitches& sw );
    virtual ~xmlpart2lilypond() {}

    Slilypondelement& current ()         { return fPartStack.top(); }
