...
int		libmxmlwrap();
bool	readfile   (const char * file, msplReader * r);
bool	readstream (FILE * file, msplReader * r);
bool	readbuffer (const char * buffer, msplReader * r);
#ifdef __cplusplus
}
#endif
...
bool readstream (FILE * fd, msplReader * r)
{
	if (!fd) return false;
	init(r);
	libmxmlrestart(fd);
...
class testmsplReader : public msplReader
{
	public:
