...
void mxmlTree2msrSkeletonBuilder::visitStart ( S_software& elt )
{
  ...
  string softwareValueToLower = stringToLowerCase (softwareValue);
  ...
  if (softwareValueToLower.find ("cubase") != string::npos) {
    ...
    if (gGlobalMxmlTree2msrOahGroup->getCubase ()) {
      S_oahElement
        cubaseOption =
      ...
      gGlobalMxmlTree2msrOahGroup->setCubase (); // JMI
    }
  }
...
*/
  ...
  if (fThereAreFiguredBassToBeAttachedToCurrentNote) {
    if (gGlobalMxmlTree2msrOahGroup->getOmitFiguredBasses ()) {
#ifdef TRACING_IS_ENABLED
