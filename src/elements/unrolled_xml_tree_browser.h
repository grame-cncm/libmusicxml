...
\brief An xml tree browser that "unroll" the score.
  ...
  These iterators are expected to contains S_measure iterators only.
  This is ensured by the visit(S_part) method.

  When visiting measures, forwarding the visit to the embedded visitor
  depends on the current state. 
  In particular, it is verified that the current time a given measure is played
  corresponds to the expected  time (if any),
  and that we're not currently jumping to a coda,
  whose location could be unknown at the time of the jump.
  ...
  public visitor<S_sound>
{
  private:
    ...
    bool  fForward;   ///< a boolean to control forwarding
                      /// of the current measure visit to the visitor
