...
void unrolled_xml_tree_browser::browse (xmlelement& t)
  { t.acceptIn(*this); }
...
void unrolled_xml_tree_browser::visitStart (S_measure& elt)
{
  ...
  if (fJump.current != kToCoda) {
    // check that we're not looking for the coda sign
    // and check that we're playing the correct round
    if (
        (fExpectedRound.current == kNoInstruction)
          ||
        (fRound.current == fExpectedRound.current))
      forwardBrowse (*elt);
  }
}
...
// equally work on a timewise score, provided that the jump information
...
// to any ctree<xmlelement>::literator.
// Using fStoreDelay allows for delayed stores.
//
void unrolled_xml_tree_browser::visitStart (S_part& elt)
{
  ...
  // while we're not at the end location (elements().end()
  // is checked for safety reasons only
...
void unrolled_xml_tree_browser::visitStart (S_sound& elt)
{
  ...
  if (!elt->getAttributeValue("segno").empty())
    { fStoreIterator = &fSegnoIterator; }
    
  if (!elt->getAttributeValue("coda").empty())
    { fJump.current = kNoJump; fStoreIterator = &fCodaIterator; }
    
  if (!elt->getAttributeValue("fine").empty())
    { fStoreDelay = 1; fStoreIterator = &fEndIterator; }
