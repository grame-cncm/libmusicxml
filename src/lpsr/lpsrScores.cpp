    ...
    : lpsrElement (inputLineNumber)
{
  ...
  fLilypondVersion =
    lpsrVarValAssoc::create (
      ...
      globalLpsrOah->fLilyPondVersion,
  ...
  // be generated?
  if (globalLpsr2LilypondOah->fXml2lyInfos) {
    // create the 'input source name and translation date' comment
    {
      ...
      if (globalOahOah->fInputSourceName == "-") {
        ...
        s << "\"" << globalOahOah->fInputSourceName << "\"";
      }

      s <<
        endl <<
        "% on " << globalGeneralOah->fTranslationDateFull <<
        endl;
  ...
  // too early to benefit from globalLpsrOah->fGlobalStaffSize... JMI
  ...
  fScoreGlobalStaffSizeSchemeVariable =
    lpsrSchemeVariable::create (
      ...
      to_string (globalLpsrOah->fGlobalStaffSize),
  ...
  if (globalLpsr2LilypondOah->fLilypondCompileDate) {
    // create the date and time functions
    addDateAndTimeSchemeFunctionsToScore ();
  }

  if (globalLpsr2LilypondOah->fPointAndClickOff) {
...
/* JMI
  ...
  msrLength paperWidth =
    elt->getPaperWidth ();
//  if (globalLpsrOah->fPaperWidth.getLengthValue () > 0.0) {
  if (globalLpsrOah->fPaperWidthAtom->getVariableHasBeenSet ()) {
    paperWidth = globalLpsrOah->fPaperWidth;
  }
  paper ->
  ...
  msrLength paperHeight =
    elt->getPaperHeight ();
//  if (globalLpsrOah->fPaperHeight.getLengthValue () > 0.0) {
  if (globalLpsrOah->fPaperHeightAtom->getVariableHasBeenSet ()) {
    paperWidth = globalLpsrOah->fPaperHeight;
  }
  paper->
  ...
  msrLength topMargin =
    elt->getTopMargin ();
  if (globalLpsrOah->fTopMargin > 0.0) {
    topMargin = globalLpsrOah->fTopMargin;
  }
  paper->
  ...
  msrLength bottomMargin =
    elt->getBottomMargin ();
  if (globalLpsrOah->fBottomMargin > 0.0) {
    bottomMargin = globalLpsrOah->fBottomMargin;
  }
  paper->
  ...
  msrLength leftMargin =
    elt->getLeftMargin ();
  if (globalLpsrOah->fLeftMargin > 0.0) {
    leftMargin = globalLpsrOah->fLeftMargin;
  }
  paper->
  ...
  msrLength rightMargin =
    elt->getRightMargin ();
  if (globalLpsrOah->fRightMargin > 0.0) {
    rightMargin = globalLpsrOah->fRightMargin;
  }
  paper->
...
*/
  ...
  // indents
  if (globalLpsrOah->fPaperHorizontalShift.getLengthValue () > 0.0) {
    fScorePaper->
      setHorizontalShift (
        msrLength::create (
          globalLpsrOah->fPaperHorizontalShift.getLengthUnitKind (),
          globalLpsrOah->fPaperHorizontalShift.getLengthValue ()));
  }

  if (globalLpsrOah->fPaperIndent.getLengthValue () > 0.0) {
    fScorePaper->
      setIndent (
        msrLength::create (
          globalLpsrOah->fPaperIndent.getLengthUnitKind (),
          globalLpsrOah->fPaperIndent.getLengthValue ()));
  }

  if (globalLpsrOah->fPaperShortIndent.getLengthValue () > 0.0) {
    fScorePaper->
      setShortIndent (
        msrLength::create (
          globalLpsrOah->fPaperShortIndent.getLengthUnitKind (),
          globalLpsrOah->fPaperShortIndent.getLengthValue ()));
  }

  // spaces
  if (globalLpsrOah->fMarkupSystemSpacingPadding.getLengthValue () > 0.0) {
    fScorePaper->
      setMarkupSystemSpacingPadding (
        msrLength::create (
          globalLpsrOah->fMarkupSystemSpacingPadding.getLengthUnitKind (),
          globalLpsrOah->fMarkupSystemSpacingPadding.getLengthValue ()));
  }

  if (globalLpsrOah->fBetweenSystemSpace.getLengthValue () > 0.0) {
    fScorePaper->
      setBetweenSystemSpace (
        msrLength::create (
          globalLpsrOah->fBetweenSystemSpace.getLengthUnitKind (),
          globalLpsrOah->fBetweenSystemSpace.getLengthValue ()));
  }

  if (globalLpsrOah->fPageTopSpace.getLengthValue () > 0.0) {
    fScorePaper->
      setPageTopSpace (
        msrLength::create (
          globalLpsrOah->fPageTopSpace.getLengthUnitKind (),
          globalLpsrOah->fPageTopSpace.getLengthValue ()));
  }

  // counts
  if (globalLpsrOah->fPageCount > 0) {
    fScorePaper->
      setPageCount (globalLpsrOah->fPageCount);
  }

  if (globalLpsrOah->fSystemCount > 0) {
    fScorePaper->
      setSystemCount (globalLpsrOah->fSystemCount);
  }
...
R"(
  ...
  // create the 'myBreakIsBreak' assoc
  {
    lpsrVarValAssoc::lpsrCommentedKind
      commentedKind =
        globalLpsr2LilypondOah->fIgnoreMusicXMLLineBreaks
  ...
  if (globalLpsr2LilypondOah->fGlobal) {
...
#ifdef TRACE_OAH
  if (globalLpsrOah->fTraceSchemeFunctions) {
    gLogOstream <<
...
chExceptionMusic = {)###" <<
    ...
    if (globalLpsr2LilypondOah->fJazzChordsDisplay) {
      s <<
        globalLpsr2LilypondOah->fJazzChordsDisplayLilypondcode;
    }

  list<pair<string, string> >&
    chordsDisplayList =
      globalLpsr2LilypondOah->fChordsDisplayList;
...
#(define (MyLedgerLineSpannerPrint grob)
  ...
  s <<
    globalLpsr2LilypondOah->fLedgerLinesRGBColor.getR () <<
    " " <<
    globalLpsr2LilypondOah->fLedgerLinesRGBColor.getG () <<
    " " <<
    globalLpsr2LilypondOah->fLedgerLinesRGBColor.getB ();
